...
const SidebarLeft = () => {
    ...
    return (
        <aside className='w-[350px] h-full py-10 px-6'>
            <article className=' flex flex-col'>
                <h1 className='text-3xl font-bold text-center'>GoMeal<span className='text-primary'>.</span></h1>
                <div className='pt-10'>
                    {MENU.map((item, index) => (
                        <Link href={item.path} className={`${pathname === item.path ? `bg-primary rounded-lg text-white` : ``} flex items-center my-5 gap-x-4 py-2 px-4 whitespace-nowrap`}>
                            <i className={`w-8 ${pathname === item.path ? 'white-icon' : ''}`}>{item.icon}</i>
                            <p>{item.name}</p>
                        </Link>
                    ))}
                </div>
