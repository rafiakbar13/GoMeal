...
import { SettingSchema, SettingSchemaType } from '@/src/common/lib/types'
import { zodResolver } from '@hookform/resolvers/zod'
import { Input } from '@/src/common/components/ui/input'
import { Label } from '@/src/common/components/ui/label'
import { Button } from '@/src/common/components/ui/button'
const Settings = () => {
    const { register, handleSubmit, formState: { errors } } = useForm<SettingSchemaType>({
    ...
    return (
        <section className='m-10 flex flex-col items-center justify-center h-screen'>
            ...
            {/* Form */}
            <form onSubmit={handleSubmit(onSubmit)} className='grid grid-cols-2 w-full items-center gap-x-6 mx-auto gap-y-10 mt-10'>
                <div className="mb-5">
                    <Label htmlFor="fullname" className='text-[14px]'>Fullname</Label>
                    <Input
                        {...register("fullname")}
                        type="text"
                    ...
                    />
                    {errors.fullname?.message && <p className="text-red-500 text-[14px]">{`${errors.fullname.message}`}</p>}
                </div>
                <div className="mb-5">
                ...
                <div className="mb-5">
                    ...
                    />
                    {errors.phone?.message && <p className="text-red-500 text-[14px]">{`${errors.phone.message}`}</p>}
                </div>

                <Button type="submit" className='text-white py-[15px] px-[35px] rounded-md mt-4 w-full shadow'>Save Changes</Button>
            </form>
        </section>
    )
}
